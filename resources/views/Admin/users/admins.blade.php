    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                                ...
                                                <td class="text-center">
                                                    @if ($admin->image)
                                                        <img class="bg-soft-primary rounded img-fluid avatar-40 me-3"
                                                            src="{{ asset('Admin/images/admins/' . $admin->image) }}"
                                                            alt="{{ $admin->name }}">
                                                    @else
                                                        <img class="bg-soft-primary rounded img-fluid avatar-40 me-3"
                                                    ...
                                                    <td>
                                                        <div class="flex align-items-center list-user-action">
                                                            ...
                                                            <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="modal"
                                                                ...
                                                                onclick="myFunction(this, '{{ $admin->id }}' ,'{{ $admin->name }}', '{{ $admin->phone }}' , '{{ $admin->email }}', '{{ $admin->country_id }}' ,'update')"
                                                                href="#editModal">
                                                            ...
                                                            <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                                ...
                                                                onclick="myFunction(this, '{{ $admin->id }}' ,'{{ $admin->name }}', '{{ $admin->phone }}' , '{{ $admin->email }}', '{{ $admin->country_id }}' ,'delete')"
                                                                href="#deleteModal">
                                                                <span class="btn-inner">
                                                                    <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                        xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <form id="submit-form" class="was-validated" action="{{ route('superAdmin.store') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="roleId" value="{{ $roleId }}">
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">
                                        {{ __('dashboard.adminName') }}
                                    </label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                        placeholder="{{ __('dashboard.adminName') }}" name="name" type="text"
                                        maxlength="200" required value="{{ old('name') }}">

                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea"
                                        class="form-label">{{ __('dashboard.adminEmail') }}</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                        placeholder="{{ __('dashboard.adminEmail') }}" name="email" type="email"
                                        required value="{{ old('email') }}">

                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea"
                                        class="form-label">{{ __('dashboard.phoneNumberAdmin') }}</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                        placeholder="{{ __('dashboard.phoneNumberAdmin') }}" maxlength="10"
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModal">{{ __('dashboard.updateadmin') }}
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <form id="update-form" class="was-validated" action="{{ route('superAdmin.update') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="country_id" id="role_id" class="form-select" required
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModal">
                            {{ __('dashboard.deleteadmin') }}
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('superAdmin.delete') }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
        ...
        <script>
            function myFunction(e, id, name, phone, email, role_id, type = "") {
            ...
            function ExportToExcel(type, fn, dl) {
                ...
                return dl ?
                    ...
                    XLSX.writeFile(wb, fn || ('المشرفين.' + (type || 'xlsx')));
            }
        </script>
    @endsection
