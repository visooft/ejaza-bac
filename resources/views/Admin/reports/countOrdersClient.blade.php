...
@section('main')
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="flex-wrap d-flex justify-content-between align-items-center">
                        <div>
                            <h1>{{ __('dashboard.countOrdersClient') }}</h1>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="overflow-hidden card" data-aos="fade-up" data-aos-delay="600">
                            <div class="flex-wrap card-header d-flex justify-content-between">
                                <div class="header-title">
                                    <h4 class="mb-2 card-title">{{ __('dashboard.bestusers') }}</h4>
                                </div>
                                <div class="dropdown">
                                    <a class="btn btn-primary" data-bs-toggle="modal" data-placement="top"
                                        title="bestusers" href="#bestusers">{{ __('dashboard.reports') }}</a>
                                </div>
                                <div class="dropdown">
                                            ...
                                            @foreach ($users as $key => $user)
                                                <tr class="item" aria-valuenow="{{ $key }}">
                                                    ...
                                                    <td class="text-center">
                                                        <img class="image bg-soft-primary rounded img-fluid avatar-40 me-3"
                                                            src="{{ $user->image }}" alt="{{ $user->user->name }}">
                                                    </td>
                                                    <td class="name">
                                                        <a href="{{ route('getprofileUser', $user->user->id) }}">
                                                            {{ $user->user->name }}
                                                        </a>
                                                    </td>
                                                    <td class="email">{{ $user->user->email }}</td>
                                                    <td class="phone">{{ $user->user->phone }}</td>
                                                    <td class="count">{{ $user->count }}</td>
                                                    <td>
                                                        <div class="flex align-items-center list-user-action">

                                                            <a class="btn btn-sm btn-icon" style="background-color:#3a57e8"
                                                            href="{{ route('getprofileUser', $user->user->id) }}">
    ...
    <div class="modal fade" id="bestusers" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('dashboard.reportUsers') }}</h5>
                        ...
                        <form id="submit-form" class="was-validated" action="{{ route('productReport') }}"
                            ...
                            <div class="form-group">
                                <label for="validationTextarea"
                                    class="form-label">{{ __('dashboard.countUsers') }}</label>
                                <input class="form-control is-invalid" id="validationTextarea"
                                    placeholder="{{ __('dashboard.countUsers') }}" type="number" min="0"
                                    step="1" name="countUsers" required>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        if (e) {
            $.post("/filterClient", {
                ...
                if (data.length > 0) {
                    ...
                    document.getElementById("user-list-table").deleteRow(data.length + 1);
                }
            })
        }
    }
