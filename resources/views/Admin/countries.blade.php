    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                ...
                                <table id="user-list-table" class="table table-striped" role="grid"
                                    ...
                                    @foreach ($countries as $key => $country)
                                        <tr class="item">
                                            <td>{{ $key + 1 }}</td>
                                            <td>
                                                <img src="{{ asset('Admin/images/countries/' . $country->flag) }}"
                                                     alt="image" class="img-fluid avatar-50">
                                            </td>
                                            <td><a href="{{ route('cities', $country->id) }}">{{ $country->name }}</a>
                                            </td>
                                            <td>
                                                {{ $country->currency }}
                                            </td>
                                                ...
                                                <td>
                                                    <div class="flex align-items-center list-user-action">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-warning"
                                                           ...
                                                           onclick="myFunction(this, '{{ $country->id }}', '{{ $country->name_ar }}' , '{{ $country->name_en }}', '{{ $country->name_tr }}', '{{ $country->currency }}' ,'update')"
                                                           href="#editModal">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                           ...
                                                           onclick="myFunction(this, '{{ $country->id }}', '{{ $country->name_ar }}' , '{{ $country->name_en }}', '{{ $country->name_tr }}', '{{ $country->currency }}' ,'delete')"
                                                           href="#deleteModal">
                                                                <span class="btn-inner">
                                                                    <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                         xmlns="http://www.w3.org/2000/svg"
                                                        ...
                                                        @if ($country->status == 1)
                                                            <a class="btn btn-sm btn-icon"
                                                               ...
                                                               onclick="myFunction(this, '{{ $country->id }}', '{{ $country->name_ar }}' , '{{ $country->name_en }}', '{{ $country->name_tr }}', '{{ $country->currency }}', 'hide')"
                                                               href="#hideSlider">
                                                                    <span class="btn-inner">
                                                        ...
                                                        @else
                                                            <a class="btn btn-sm btn-icon"
                                                               ...
                                                               onclick="myFunction(this, '{{ $country->id }}', '{{ $country->name_ar }}' , '{{ $country->name_en }}', '{{ $country->name_tr }}', '{{ $country->currency }}', 'show')"
                                                               href="#showSlider">
                                                                    <span class="btn-inner">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">اضافه دولة</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="submit-form" class="was-validated" action="{{ route('country.store') }}"
                                ...
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">العملة</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                           placeholder="العملة" type="text" name="currency" required>
                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">كود الدولة</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                           placeholder="الكود" type="text" name="code" required maxlength="5"
                                           minlength="2">
                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">علم الدولة</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                           placeholder="العلم" type="file" name="flag" required>
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModal">تحديث الدولة</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="update-form" class="was-validated" action="{{ route('country.update') }}"
                                  method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <input id="cityId" name="cityId" type="hidden" class="form-control">
                                <div class="form-group">
                                    <label for="validationTextarea"
                                           class="form-label">{{ __('dashboard.name_ar') }}</label>
                                ...
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">الاسم بالتركي</label>
                                    <input class="form-control is-invalid" id="name_tr" placeholder="الاسم بالتركي"
                                           type="text" name="name_tr" required>

                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">العملة</label>
                                    <input class="form-control is-invalid" id="currency"
                                           placeholder="العملة" type="text" name="currency" required>
                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">كود الدولة</label>
                                    <input class="form-control is-invalid" id="code"
                                           placeholder="كود" type="text" name="code" required>
                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">علم الدولة</label>
                                    <input class="form-control is-invalid" id="flag"
                                           placeholder="العلم" type="file" name="flag">
                                </div>
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('country.delete') }}"
                              method="POST">
                            @csrf
                            @method('DELETE')
                            <input id="cityIdDelete" name="cityId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <div class="modal fade" id="showSlider" tabindex="-1" aria-labelledby="showSlider" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="showSlider">اظهار الدولة</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="show-form" class="was-validated" action="{{ route('country.show') }}" method="POST">
                            @csrf
                            @method('put')
                            <input id="sliderIdShow" name="cityId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <div class="modal fade" id="hideSlider" tabindex="-1" aria-labelledby="hideSlider" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="hideSlider">اخفاء الدولة</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="hide-form" class="was-validated" action="{{ route('country.hide') }}" method="POST">
                            @csrf
                            @method('put')
                            <input id="sliderIdHide" name="cityId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <script>
            function myFunction(e, id, name_ar, name_en, name_tr, currency, type = "") {
                if (type == "update") {
                    const e = document.getElementById('cityId');
                    if (e) {
                        e.value = id;
                    ...
                    const e = document.getElementById('cityIdDelete');
                    if (e) e.value = id;
                } else if (type == "show") {
