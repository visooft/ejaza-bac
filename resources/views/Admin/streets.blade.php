    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                                    ...
                                                    <td>
                                                        <div class="flex align-items-center list-user-action">
                                                            ...
                                                            <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="modal"
                                                                ...
                                                                onclick="myFunction(this, '{{ $street->id }}', '{{ $street->name_ar }}' , '{{ $street->name_en }}', '{{ $street->name_tr }}', '{{ $street->lat }}', '{{ $street->long }}' ,'update')"
                                                                href="#editModal">
                                                            ...
                                                            <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                                ...
                                                                onclick="myFunction(this, '{{ $street->id }}', '{{ $street->name_ar }}' , '{{ $street->name_en }}', '{{ $street->name_tr }}', '{{ $street->lat }}', '{{ $street->long }}' ,'delete')"
                                                                href="#deleteModal">
                                                                <span class="btn-inner">
                                                                    <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                        xmlns="http://www.w3.org/2000/svg"
                                                            ...
                                                            @if ($street->status == 1)
                                                                <a class="btn btn-sm btn-icon"
                                                                    ...
                                                                    onclick="myFunction(this, '{{ $street->id }}', '{{ $street->name_ar }}' , '{{ $street->name_en }}', '{{ $street->name_tr }}', '{{ $street->lat }}', '{{ $street->long }}', 'hide')"
                                                                    href="#hideSlider">
                                                                    <span class="btn-inner">
                                                            ...
                                                            @else
                                                                <a class="btn btn-sm btn-icon"
                                                                    ...
                                                                    onclick="myFunction(this, '{{ $street->id }}', '{{ $street->name_ar }}' , '{{ $street->name_en }}', '{{ $street->name_tr }}', '{{ $street->lat }}', '{{ $street->long }}', 'show')"
                                                                    href="#showSlider">
                                                                    <span class="btn-inner">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">اضافة حي</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="submit-form" class="was-validated" action="{{ route('street.store') }}"
                                ...
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">الطول</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                        placeholder="الطول" type="text" name="lat" required>

                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">االعرض</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                        placeholder="االعرض" type="text" name="long" required>
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModal">تعديل حي</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="update-form" class="was-validated" action="{{ route('street.update') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <input id="street_id" name="street_id" type="hidden" class="form-control">
                                <div class="form-group">
                                    <label for="validationTextarea"
                                        class="form-label">{{ __('dashboard.name_ar') }}</label>
                                ...
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">الطول</label>
                                    <input class="form-control is-invalid" id="lat"
                                        placeholder="الطول" type="text" name="lat" required>

                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">االعرض</label>
                                    <input class="form-control is-invalid" id="long"
                                        placeholder="االعرض" type="text" name="long" required>
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModal">حذف حي</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('street.delete') }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                            <input id="street_idDelete" name="street_id" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <div class="modal fade" id="showSlider" tabindex="-1" aria-labelledby="showSlider" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="showSlider">اظهار الحي</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="show-form" class="was-validated" action="{{ route('street.show') }}" method="POST">
                            @csrf
                            @method('put')
                            <input id="sliderIdShow" name="street_id" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <div class="modal fade" id="hideSlider" tabindex="-1" aria-labelledby="hideSlider" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="hideSlider">اخفاء الحي</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="hide-form" class="was-validated" action="{{ route('street.hide') }}" method="POST">
                            @csrf
                            @method('put')
                            <input id="sliderIdHide" name="street_id" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <script>
            function myFunction(e, id, name_ar, name_en, name_tr, lat, long, type = "") {
                if (type == "update") {
                    const e = document.getElementById('street_id');
                    if (e) {
                        e.value = id;
                    ...
                    const e = document.getElementById('street_idDelete');
                    if (e) e.value = id;
                } else if (type == "show") {
