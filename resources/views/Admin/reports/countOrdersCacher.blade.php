...
@section('main')
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="flex-wrap d-flex justify-content-between align-items-center">
                        <div>
                            <h1>{{ __('dashboard.countOrdersCacher') }}</h1>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <li class="swiper-slide card card-slide" data-aos="fade-up" data-aos-delay="1300">
                                    <div class="card-body">
                                        <div class="progress-widget">
                                            ...
                                            <div class="progress-detail">
                                                <p class="mb-2">{{ __('dashboard.cacherCount') }}</p>
                        ...
                        <div class="overflow-hidden card" data-aos="fade-up" data-aos-delay="600">
                            <div class="flex-wrap card-header d-flex justify-content-between">
                                <div class="header-title">
                                    <h4 class="mb-2 card-title">{{ __('dashboard.bestCacher') }}</h4>
                                </div>
                                <div class="dropdown">
                                    <a class="btn btn-primary" data-bs-toggle="modal" data-placement="top"
                                        title="bestCacher" href="#bestCacher">{{ __('dashboard.reports') }}</a>
                                </div>
                            </div>
                                        ...
                                        <tbody>
                                            @foreach ($usersData as $key => $user)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a
                                                            href="{{ route('getProfile', $user->id) }}">{{ $user->name }}</a>
                                                    </td>
                                                    <td>{{ $user->email }}</td>
                                                    <td>{{ $user->phone }}</td>
                                                    <td>{{ $user->count }}</td>
                                                    <td>
                                                        <div class="flex align-items-center list-user-action">
                                                            <a class="btn btn-sm btn-icon"
                                                                style="background-color:#3a57e8"
                                                                href="{{ route('getProfile', $user->id) }}">
    ...
    <div class="modal fade" id="bestCacher" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('dashboard.reportCacher') }}</h5>
                        ...
                        <form id="submit-form" class="was-validated" action="{{ route('productReport') }}"
                            ...
                            <div class="form-group">
                                <label for="validationTextarea"
                                    class="form-label">{{ __('dashboard.cacherCount') }}</label>
                                <input class="form-control is-invalid" id="validationTextarea"
                                    placeholder="{{ __('dashboard.cacherCount') }}" type="number" min="0"
                                    step="1" name="cacherCount" required>
                            </div>
                        </form>
                    </div>
                </div>
