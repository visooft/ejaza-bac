    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                        ...
                                        <tbody>
                                            @foreach ($offers as $key => $offer)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                                    <td>
                                                        {{ $offer->ad->category->name_ar }}
                                                    </td>
                                                    <td>{{ $offer->ad->name_ar }}</td>
                                                    <td>{{ $offer->offer }}%</td>
                                                    @can('superAdmin')
                                                        <td>
                                                            <div class="flex align-items-center list-user-action">
                                                                <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                                    ...
                                                                    onclick="myFunction(this, '{{ $offer->id }}','delete')"
                                                                    href="#deleteModal">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{ __('dashboard.addOffer') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="submit-form" class="was-validated" action="{{ route('offers.store') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">{{ __('dashboard.offer') }}</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                        placeholder="{{ __('dashboard.offer') }}" type="number" min="0"
                                        max="100" step="1" name="offer" required>
                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">القسم</label>
                                    <select name="category_id" id="key" class="form-select" required
                                        aria-label="select example" onclick="getProducts(this.value)">
                                ...
                                <div class="form-group" id="product_idDiv" style="display: none">
                                    <label for="validationTextarea" class="form-label">المنتج</label>
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModal">{{ __('dashboard.deleteOffer') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('offers.delete') }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                            <input id="categoryIdDelete" name="offerId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <script>
            ...
            function getProducts(shopId) {
                let productDiv = document.getElementById("product_id")
                let regionId = document.getElementById("product_idDiv")
                ...
                if (shopId) {
                    $.post("/offers/getProducts", {
                        ...
                        if (Array.isArray(data)) {
                            document.getElementById("product_idDiv").style.display = "block"
