    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                ...
                                <table id="user-list-table" class="table table-striped" role="grid"
                                    ...
                                    @foreach ($users as $key => $user)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if ($user->image)
                                                    <img class="bg-soft-primary rounded img-fluid avatar-40 me-3"
                                                         src="{{ asset('Admin/images/users/' . $user->image) }}"
                                                         alt="{{ $user->name }}">
                                                @else
                                                    <img class="bg-soft-primary rounded img-fluid avatar-40 me-3"
                                            ...
                                            <td><a
                                                        href="{{ route('getprofileUser', $user->id) }}">{{ $user->name }}</a>
                                            </td>
                                            <td>{{ $user->email }}</td>
                                            <td>{{ $user->phone }}</td>
                                            <td>{{ $user->country }}</td>
                                            <td>{{ $user->wallet }} {{ __('dashboard.Rial') }}</td>
                                            <td>
                                                @if ($user->status != 3)
                                                ...
                                                @else
                                                    <a class="btn btn-sm btn-icon btn-success"
                                                       href="{{ route('unBlock', $user->id) }}">
                                                        {{ __('dashboard.unBlock') }}</a>
                                                @endif
                                            </td>
                                            <td>
                                                @if ($user->documentation == 0)
                                                    <a class="btn btn-sm btn-icon btn-danger"
                                                       href="{{ route('Accountdocumentation', $user->id) }}">
                                                        {{ __('dashboard.documentation') }}</a>
                                                @else
                                                    <a class="btn btn-sm btn-icon btn-success"
                                                       href="{{ route('Accountdocumentation', $user->id) }}">
                                                        {{ __('dashboard.unDocumentation') }}</a>
                                                @endif
                                            </td>
                                                ...
                                                <td>
                                                    <div class="flex align-items-center list-user-action">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-warning"
                                                           ...
                                                           onclick="myFunction(this, '{{ $user->id }}' ,'{{ $user->name }}', '{{ $user->phone }}' , '{{ $user->email }}', '{{ $user->image }}', '{{ $user->country_id }}' ,'update')"
                                                           href="#editModal">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                           ...
                                                           onclick="myFunction(this, '{{ $user->id }}' ,'{{ $user->name }}', '{{ $user->phone }}' , '{{ $user->email }}', '{{ $user->image }}', '{{ $user->country_id }}' ,'delete')"
                                                           href="#deleteModal">
                                                                <span class="btn-inner">
                                                                    <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                         xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <form id="submit-form" class="was-validated" action="{{ route('user.store') }}"
                                  method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="roleId" value="{{ $roleId }}">
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">
                                        {{ __('dashboard.memberName') }}
                                    </label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                           placeholder="{{ __('dashboard.memberName') }}" name="name" type="text"
                                           maxlength="200" required>

                                </div>
                                <div class="form-group">
                                    <label for="validationTextarea"
                                           class="form-label">{{ __('dashboard.memberEmail') }}</label>
                                    <input class="form-control is-invalid" id="validationTextarea"
                                           placeholder="{{ __('dashboard.memberEmail') }}" name="email" type="email"
                                           required>
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModal">{{ __('dashboard.memberUpdate') }}
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <form id="update-form" class="was-validated" action="{{ route('user.update') }}"
                                  method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModal">
                            {{ __('dashboard.deleteMember') }}
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('user.delete') }}"
                              method="POST">
                            @csrf
                            @method('DELETE')
        ...
        <script>
            function myFunction(e, id, name, phone, email, image, country_id, type = "") {
                if (type == "update") {
                    ...
                    console.log(id);
                    const e = document.getElementById('userIdForMessage');
                    if (e) e.value = id;
                }
            }
            ...
            function ExportToExcel(type, fn, dl) {
                ...
                return dl ?
                    ...
                    XLSX.writeFile(wb, fn || ('الاعضاء الجدد.' + (type || 'xlsx')));
            }
        </script>
@endsection
