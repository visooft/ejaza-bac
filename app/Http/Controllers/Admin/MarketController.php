...
class MarketController extends Controller
{
    ...
    public function market($status)
    {
        $ads = $this->houseModel::where(['category_id' => 9, 'status' => $status])->orderBy('id', 'DESC')->paginate(10);
        foreach ($ads as $ad) {
            $ad->image = env('APP_URL') . 'Admin/images/ads/' . $this->imageModel::where('travel_id', $ad->id)->first()->image;
        }
        return view('Admin.resturants.market', compact('ads', 'status'));
    }
