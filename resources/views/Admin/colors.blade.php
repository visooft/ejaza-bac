    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                    ...
                                    @can('superAdmin')
                                        <tbody>
                                            @foreach ($data as $key => $color)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                                    <td style="background-color: {{ $color->code }}">{{ $color->name }}</td>
                                                    @can('superAdmin')
                                                        <td>{{ $color->shopName }}</td>
                                                    @endcan
                                                    @if (auth()->user()->role->name == 'Employee')
                                                        <td>{{ $color->shopName }}</td>
                                                    @endif
                                                        ...
                                                        <td>
                                                            <div class="flex align-items-center list-user-action">
                                                                ...
                                                                <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="modal"
                                                                    ...
                                                                    onclick="myFunction(this, '{{ $color->id }}', '{{ $color->color_id }}','update')"
                                                                    href="#editModal">
                                                                ...
                                                                <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                                    ...
                                                                    onclick="myFunction(this, '{{ $color->id }}', '{{ $color->color_id }}','delete')"
                                                                    href="#deleteModal">
                                                                    <span class="btn-inner">
                                                                        <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                            xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    @can('merchant')
                                        <tbody>
                                            @foreach ($data as $key => $color)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                                    <td width="20px" style="background-color: {{ $color->code }}">
                                                        {{ $color->name }}</td>
                                                    @can('superAdmin')
                                                        <td>{{ $color->shopName }}</td>
                                                    @endcan
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{ __('dashboard.addColor') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="submit-form" class="was-validated" action="{{ route('colors.store') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    <label for="validationTextarea"
                                        class="form-label">{{ __('dashboard.color') }}</label>
                                    <select name="colorId" class="form-select" required aria-label="select example">
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModal">{{ __('dashboard.updateColor') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="update-form" class="was-validated" action="{{ route('colors.update') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <input id="categoryId" name="colorId" type="hidden" class="form-control">
                                <div class="form-group">
                                    <label for="validationTextarea"
                                        class="form-label">{{ __('dashboard.color') }}</label>
                                    <select name="colorDataId" id="colorDataId" class="form-select" required
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModal">{{ __('dashboard.delteColor') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('colors.delete') }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                            <input id="categoryIdDelete" name="colorId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <script>
            function myFunction(e, id, color_id, type = "") {
                if (type == "update") {
                    const e = document.getElementById('categoryId');
                    if (e) {
                        e.value = id;
                        document.getElementById('colorDataId').value = color_id;
                    }
                } else if (type == "delete") {
