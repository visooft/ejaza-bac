    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                ...
                                @can('merchant')
                                    <div class="d-inline flex-fill p-4 justify-content-between flex-wrap">
                                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                            ...
                                            @if ($shop->category->name_en == 'Resturant')
                                                {{ __('dashboard.addSize') }}
                                            @else
                                                {{ __('dashboard.addmeasurement') }}
                                            @endif
                                        </button>
                                    </div>
                                @endcan
                                                        ...
                                                        <td>
                                                            <div class="flex align-items-center list-user-action">
                                                                ...
                                                                <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="modal"
                                                                    ...
                                                                    onclick="myFunction(this, '{{ $size->id }}', '{{ $size->name_ar }}' , '{{ $size->name_en }}' ,'update')"
                                                                    href="#editModal">
                                                                ...
                                                                <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                                    ...
                                                                    onclick="myFunction(this, '{{ $size->id }}', '{{ $size->name_ar }}' , '{{ $size->name_en }}' ,'delete')"
                                                                    href="#deleteModal">
                                                                    <span class="btn-inner">
                                                                        <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                            xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <form id="submit-form" class="was-validated" action="{{ route('size.store') }}"
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModal">{{ __('dashboard.updateSize') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="update-form" class="was-validated" action="{{ route('size.update') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <input id="categoryId" name="sizeId" type="hidden" class="form-control">
                                <div class="form-group">
                                    <label for="validationTextarea"
                                        class="form-label">{{ __('dashboard.name_ar') }}</label>
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModal">{{ __('dashboard.delteSize') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('size.delete') }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                            <input id="categoryIdDelete" name="sizeId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
