                                ...
                                <table id="user-list-table" class="table table-striped" role="grid"
                                    ...
                                    <thead>
                                        <tr class="ligth">
                                            ...
                                            <th>{{ __('dashboard.addproduct') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($subMenues as $key => $menue)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                @if ($type == 1)
                                                    <td class="text-center"><img
                                                            class="bg-soft-primary rounded img-fluid avatar-40 me-3"
                                                            src="{{ asset('Admin/images/subMenues/' . $menue->image) }}"
                                                            alt="{{ $menue->name }}"></td>
                                                @endif

                                                <td><a href="{{ route('products', $menue->id) }}">{{ $menue->name }}</a>
                                                </td>
                                                <td>
                                                    <div class="flex align-items-center list-user-action">
                                                        <a class="btn btn-sm btn-icon" style="background-color:#3a57e8"
                                                            href="{{ route('products', $menue->id) }}">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="modal"
                                                            ...
                                                            onclick="myFunction(this, '{{ $menue->id }}', '{{ $menue->name_ar }}' , '{{ $menue->name_en }}','update')"
                                                            href="#editModal">
                                                            <span class="btn-inner">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                            ...
                                                            onclick="myFunction(this, '{{ $menue->id }}', '{{ $menue->name_ar }}' , '{{ $menue->name_en }}' ,'delete')"
                                                            href="#deleteModal">
                                                            <span class="btn-inner">
                                                                <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                    xmlns="http://www.w3.org/2000/svg"
                                                ...
                                                <td><a href="{{ route('products.add', $menue->id) }}"
                                                            class="btn btn-success">{{ __('dashboard.addproduct') }}</a></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <form id="submit-form" class="was-validated" action="{{ route('submenues.store') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <input name="menueId" type="hidden" class="form-control" value="{{ $id }}">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <form id="update-form" class="was-validated" action="{{ route('submenues.update') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('submenues.delete') }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
