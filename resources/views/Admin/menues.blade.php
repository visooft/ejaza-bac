    ...
    <div class="conatiner-fluid content-inner mt-n5 py-0">
                                    ...
                                    <tbody>
                                        @foreach ($menues as $key => $menue)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td><a
                                                        href="{{ route('submenues', $menue->shop_id) }}">{{ $menue->name }}</a>
                                                </td>
                                                <td>{{ $menue->shopName }}</td>
                                                <td>
                                                    <div class="flex align-items-center list-user-action">

                                                        <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="modal"
                                                            data-placement="top" title="" data-original-title="Edit"
                                                            onclick="myFunction(this, '{{ $menue->id }}', '{{ $menue->name_ar }}' , '{{ $menue->name_en }}', '{{ $menue->shop_id }}' ,'update')"
                                                            href="#editModal">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                            ...
                                                            onclick="myFunction(this, '{{ $menue->id }}', '{{ $menue->name_ar }}' , '{{ $menue->name_en }}', '{{ $menue->shop_id }}' ,'delete')"
                                                            href="#deleteModal">
                                                            <span class="btn-inner">
                                                                <svg width="20" viewBox="0 0 24 24" fill="none"
                                                                    xmlns="http://www.w3.org/2000/svg"
                                                        ...
                                                        @if ($menue->status == 1)
                                                            <a class="btn btn-sm btn-icon"
                                                                ...
                                                                onclick="myFunction(this, '{{ $menue->id }}', '{{ $menue->name_ar }}' , '{{ $menue->name_en }}', '{{ $menue->shop_id }}' ,'hide')"
                                                                href="#hidecategory">
                                                                <span class="btn-inner">
                                                        ...
                                                        @else
                                                            <a class="btn btn-sm btn-icon"
                                                                ...
                                                                onclick="myFunction(this, '{{ $menue->id }}', '{{ $menue->name_ar }}' , '{{ $menue->name_en }}', '{{ $menue->shop_id }}' ,'show')"
                                                                href="#showcategory">
                                                                <span class="btn-inner">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{ __('dashboard.addMenue') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="submit-form" class="was-validated" action="{{ route('menues.store') }}"
                                ...
                                <div class="form-group">
                                    <label for="validationTextarea" class="form-label">{{ __('dashboard.shop') }}</label>
                                    <select name="shop_id" class="form-select" required aria-label="select example">
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModal">{{ __('dashboard.updateMenue') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="card-body">

                            <form id="update-form" class="was-validated" action="{{ route('menues.update') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <input id="menueId" name="menueId" type="hidden" class="form-control">
                                <div class="form-group">
                                    <label for="validationTextarea"
                                        class="form-label">{{ __('dashboard.name_ar') }}</label>
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModal">{{ __('dashboard.delteMenue') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="delete-form" class="was-validated" action="{{ route('menues.delete') }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                            <input id="menueIdDelete" name="menueId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <div class="modal fade" id="showcategory" tabindex="-1" aria-labelledby="showcategory" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="showcategory">{{ __('dashboard.showMenue') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="show-form" class="was-validated" action="{{ route('menues.show') }}" method="POST">
                            @csrf
                            @method('put')
                            <input id="categoryIdShow" name="menueId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <div class="modal fade" id="hidecategory" tabindex="-1" aria-labelledby="hidecategory" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="hidecategory">{{ __('dashboard.hideMenue') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form id="hide-form" class="was-validated" action="{{ route('menues.hide') }}" method="POST">
                            @csrf
                            @method('put')
                            <input id="categoryIdHide" name="menueId" type="hidden" class="form-control">
                        </form>
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('dashboard.close') }}</button>
        ...
        <script>
            function myFunction(e, id, name_ar, name_en, shop_id, type = "") {
                if (type == "update") {
                    const e = document.getElementById('menueId');
                    if (e) {
                        e.value = id;
                    ...
                    const e = document.getElementById('menueIdDelete');
                    if (e) e.value = id;
                } else if (type == "show") {
