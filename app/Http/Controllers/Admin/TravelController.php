...
class TravelController extends Controller
{
    ...
    public function travel($status)
    {
        $ads = $this->houseModel::where(['category_id' => 8, 'status' => $status])->orderBy('id', 'DESC')->paginate(10);
        foreach ($ads as $ad) {
            $ad->image = env('APP_URL') . 'Admin/images/ads/' . $this->imageModel::where('travel_id', $ad->id)->first()->image;
        }
        // dd($ads[0]->travel_country->name_ar);
        return view('Admin.resturants.travels', compact('ads', 'status'));
    }
